...
export const EditProject = () => {
    ...
    const currentProjectDetails = location.state?.project;
    ...
    const submit = async (e) => {
        ...
        try {
          const result = await tryCallApiAsync(`${baseUrl}/projects/${currentProject.projectId}`, 'PUT', formData);
          if (result) {
    ...
    useEffect( () => {
      fetch(`${baseUrl}/projects/${currentProjectDetails.projectId}`)
        ...
        .then( 
          data => {
            if (data.success == true) {
              ...
              setStartDate(data.data.startDate.split("T")[0]);
              setEndDate(data.data.endDate.split("T")[0]);
              setStatusId(data.data.statusId);
              setCustomerId(data.data.customerId);
              setServiceId(data.data.serviceId);
              setProjectManagerId(data.data.projectManagerId);
            } else {
              setCurrentProject([])
            }
          }          
    ...
    <main>        
        <form onSubmit={submit}>
          ...
          <div className='input-group'>
            ...
            <input type='text' name='projectName' value={projectName} onChange={event => setProjectName(event.target.value)} />
          </div>

          <div className='input-group'>
            <label>Project Description</label>
            <input type='text-area' name='projectDescription' value={projectDescription} onChange={event => setProjectDescription(event.target.value)} />
          </div>
          ...
          <div className='input-group'>
            ...
            <select value={projectMangerId} onChange={event => setProjectManagerId(event.target.value)}>
              {
